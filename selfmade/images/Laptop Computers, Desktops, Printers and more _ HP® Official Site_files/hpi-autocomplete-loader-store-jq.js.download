...
return keys;};}
can.view.ejs('ACLoaderStoreEJS','    <ul>'+'        <% can.$.each(list, function( i, item ) { '+'          var key = Object.keys(this)[0]; %>'+'                <li>'+'                    <a id="i<%= i%>" href="javascript:void(0);" class="js_suggestion_text" data-suggestion="<%=key%>">'+'                        <%= key %>'+'                    </a>'+'                </li>'+'        <% }) %>'+'    </ul>');HP.StoreAutocompleteLoader=can.Control({defaults:{view:"ACLoaderStoreEJS",language:'',country:'',requestType:'jsonp',url:'',count:5,callbackParam:'cbf'}},{'init':function(element,options){window.customAutocomplete?this.type=window.customAutocomplete[0].type:this.type="hho-store";},'getSuggestions':function(query,callback){var params={term:query,lang:this.options.language,};var result={'query':query,'html':'','itemsCount':0};var self=this;$.ajax({url:this.options.url,data:params,success:function(data){if(data){var items=data.terms;if(items&&items.length!=0){if(self.options.count<items.length){items=items.slice(0,self.options.count);}
